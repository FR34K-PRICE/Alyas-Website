...
`check:motion` (`BASE_URL=… npm run check:motion`) measures the airplane animation and its fallbacks in a real browser; see [Motion and reduced motion](#motion-and-reduced-motion).
...
- **Checking it automatically.** `BASE_URL=http://localhost:3000 npm run check:motion` starts Chrome or Edge headless (no extra package; set `CHROME_PATH` if it is not found), and measures the real page: the flight starts, travels from the correct side in English (LTR) and Arabic (RTL), finishes once, and leaves the plane visible, still, inside the hero and clear of the headline, copy, buttons and menu button at every frame, at desktop, tablet, phone and narrow-phone widths. It also checks reduced motion, JavaScript disabled, a failing inline script, and blocked scripts. It only reads pages, so it is safe against any server. With no browser it says so and exits with code 3; that is never a pass.
- **Checking it by hand** (if the automated check cannot run): open the page in a browser where animations are allowed; the plane should fly in from the left (English) or right (Arabic) over about 3–4 seconds, land, and stay put. Then, in DevTools, *Rendering → Emulate CSS media feature prefers-reduced-motion → reduce* and reload: the plane must already be in its final place and not move. Finally disable JavaScript and reload: the plane must still be visible.

### Hero video (preview branch)

The homepage hero can play `public/video/alyas-cloud-flight.mp4` (silent, H.264, 1280×720, fast-start) over its poster image.
...
- **While the video is shown, the airplane is not** (the video replaces it), so `npm run check:motion` and the airplane assertion in `check:home` do not apply to this branch.
