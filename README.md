...
- **Public site**: home (video hero), about, travel services, events & conferences, contact. Floral services appear as a secondary ALYAS Group offer.
...
- **Design**: a travel-editorial look: a full-bleed hero (a muted looping cloud-flight video by default, or your own photograph with a flying airliner and optional foreground cut-out), an oversized headline, white pages, photographic service tiles, alternating photo compositions, Bricolage Grotesque (Latin) and Readex Pro (Arabic) headlines. Photo sources and reuse terms: [docs/IMAGE-CREDITS.md](docs/IMAGE-CREDITS.md).
...
`check:home` checks the public site: the hero copy in both languages, that contact actions come only from the details saved in the CMS, the secondary "Also from ALYAS Group" row, `/`, the old aliases and canonical links, SEO metadata, images and structured data on every public page in both languages, and the health endpoint's contents. It is read-only unless you set `HOME_CHECK_MUTATE=1`, which also proves that untouched starter wording is upgraded while edited wording is preserved exactly, and that WhatsApp/phone/email validation and normalisation agree in the hero, mobile bar, footer, contact page and structured data. It always asserts the hero's real markup for the mode the site is in (the video hero, or the photo hero with its airplane); in mutation mode it also switches the test site between the video and a CMS photograph (with a foreground cut-out and a custom airplane image), checks alt text and the fallbacks, and puts everything back. That mode temporarily saves and publishes test values in Home and Site & brand and restores your originals: **use a local or isolated test database only, never production.**

`check:motion` (`BASE_URL=… npm run check:motion`) measures the hero's motion and its fallbacks in a real browser, as two suites: the **video hero** (real frames advance, reduced motion, JavaScript off, failed inline script, blocked scripts, Play button placement, English/Arabic at four widths) and the **aircraft/photo hero** (the original airplane checks). Without credentials it runs the suite matching the site's current hero and prints `NOT RUN` for the other; with `ADMIN_EMAIL`/`ADMIN_PASSWORD` of an **isolated test site** it switches the hero between the two modes itself and restores it. See [Motion and reduced motion](#motion-and-reduced-motion).
...
- **How contact details are checked.** One set of rules ([`src/content/contact-rules.ts`](src/content/contact-rules.ts)) is used both when saving in the CMS and when rendering the hero, mobile bar, footer, contact page and structured data, so they cannot disagree:
  ...
  - The CMS shows a message under the field and will not save a value that fails these rules. **If a site already has an older value that fails (for example a WhatsApp number written as `0770…`), saving Site & brand is blocked until an administrator corrects or clears that field**; the message says which field and why. **Recovery, with no code and no database editing:** (1) sign in as an administrator; (2) open **Admin → Site & brand → Contact details**; (3) the field with the red message is the old value: rewrite it in the accepted form (for example `0770 123 4567` → `+964 770 123 4567` for WhatsApp) or clear it to hide that contact action; (4) press **Save draft**, then **Publish**. Nothing else on the page needs to change, and the public site never showed a broken link in the meantime. Until then, failing values are ignored when rendering, so no broken `tel:`, `wa.me` or `mailto:` link is ever produced.
...
`src/components/site/Hero.tsx`, `src/site/pages.tsx` (`heroProps`) and `public/video/`, `public/photos/`.

The homepage hero has **two modes**, chosen by one CMS field, **Home page → Hero → Hero photograph**:

| | **Video hero** (the default) | **Photo hero** (a CMS hero photograph is set) |
| --- | --- | --- |
| When | Hero photograph left empty | A photograph is chosen from the Media library |
| Background | Muted looping video over its poster image (see [Hero video](#hero-video-default-hero)) | Your photograph |
| Airplane | None (the video replaces it) | A real airliner flies in once and parks; "Airplane image" swaps it for your own |
| Foreground cut-out | None | Only if "Foreground cut-out" is also set (it must match the photograph's crop) |
| Header | Transparent, navy text and dark logo (the video is bright) | Transparent, white text and light logo (assumes a dark photograph) |
| Motion | The video (autoplay only where the browser allows it) | The one-time airplane flight |

A foreground cut-out or airplane image saved **without** a hero photograph changes nothing: the video stays. Clearing the hero photograph brings the video back. `npm run check:home` (mutation mode) and `npm run check:motion` (with test-site credentials) prove both modes and the switch between them.

- **Layers, video hero** (back to front): poster, video, shade, headline, copy and buttons, Play button. The shade is a smooth full-width bottom gradient above the video (it was once underneath it and darkened nothing). Measured over real video frames: copy at least 6.9:1, the large headline at least 4.0:1 (needs 3:1), navy header text at least 5.5:1 and the small gold "Travel"/"للسفر" at least 5.0:1 (needs 4.5:1), on English and Arabic, phone and desktop.
- **Layers, photo hero** (back to front): hero photograph, shade, aircraft, headline, optional **foreground cut-out**, copy and button. A soft shade behind the headline keeps it readable; replacing the photograph means re-checking contrast on the rendered page.
- **Aircraft** (photo hero): a real airliner photograph (gear retracted), cut out by modelling the sky, with airline marks painted out. Home page → Hero has an optional "Airplane image" (transparent PNG/WebP, nose pointing right, mirrored automatically in Arabic); the built-in one is the fallback. It crosses an open part of the sky once (Web Animations API, no library), then stays put. Reduced motion or no scripts: it is simply parked. It pauses offscreen; phones and weaker devices get a shorter, smaller version. No scroll hijacking, no intro screen, no sound.
- **Text layout follows the photograph** (photo hero): the headline sits on the right and the copy on the left in both languages; Arabic has its own headline scale, leading and line lengths.
- **The built-in mountain-lake photograph and its foreground cut-out are no longer used by the hero.** The photograph still serves as a fallback picture for some cards and sections, and `hero-fg-*.webp` is kept so the earlier hero can be restored. They are still produced by `npm run prepare-photos`.
- **CMS**: every image slot is replaceable (Media library). A custom hero photograph is used *without* any cut-out unless you also upload a matching one, because the two must be cropped identically.
- **Regenerating photographs**: put originals in `assets/source/` (`hero.jpg`, `coast.jpg`, `hiker.jpg`, `road.jpg`, `window.jpg`, `city.jpg`, `flowers.jpg`, `aircraft.jpg`; the aircraft mark positions in `prepare-photos.mjs` are specific to the supplied photo) and run `npm run prepare-photos`. It writes WebP sizes, the hero foreground, the aircraft cut-out and `src/site/photo-manifest.json`.

### Motion and reduced motion

The homepage's deliberate motion depends on its hero mode (see [Hero and photography](#hero-and-photography)): in the default **video hero** it is the muted looping video (see [Hero video](#hero-video-default-hero)); in the **photo hero** (a CMS hero photograph is set) it is the airliner that flies in once and then stays parked. Smaller motion in both: card photos zoom slightly on hover, smooth scrolling, and short colour/underline transitions.

- **How the airplane works (photo hero).** A tiny inline script in `src/app/layout.tsx` adds `js-motion` to `<html>` only when the visitor has *not* asked for reduced motion. With `js-motion`, the plane starts hidden and `Hero.tsx` flies it in with the Web Animations API (about 3.6 s on desktop, 1.8 s on phones and weaker devices), pausing while the hero is off-screen. Without `js-motion` (reduced motion, JavaScript off, or the script failing) the plane is simply a visible static image at its parked spot. If the page's scripts never run after `js-motion` was set, a CSS rule reveals the parked plane after 5 seconds, so it can never stay hidden.
- **Reduced motion is respected, on purpose.** With `prefers-reduced-motion: reduce` the video does not autoplay (the poster and a Play button are shown instead), the plane does not move, hover zoom and smooth scrolling are off, and transitions are near-instant.
- **"The animation stopped working" is usually this setting.** Browsers report `prefers-reduced-motion: reduce` when the operating system has animations turned off (Windows: *Settings → Accessibility → Visual effects → Animation effects*, or the classic *System Properties → Performance Options → Animate controls and elements*; macOS: *Accessibility → Display → Reduce motion*), and some tweak utilities, remote-desktop sessions and battery-saver modes switch it off. To check, run `matchMedia('(prefers-reduced-motion: reduce)').matches` in the browser's console: `true` means the site is correctly keeping the plane still. Turn animations back on in the OS, then reload the page.
- **Checking it automatically.** `BASE_URL=http://localhost:3000 npm run check:motion` starts Chrome or Edge headless (no extra package; set `CHROME_PATH` if it is not found) and measures the real page. **Video hero suite:** real decoded frames advance with motion allowed; with reduced motion nothing plays, the poster and a Play button show and a tap starts real frames; the Play button sits on the correct side (LTR/RTL) and never touches the headline, copy, buttons, header or menu button; JavaScript off, a failing inline script and blocked scripts leave the poster, native controls and text visible; hover zoom and smooth scrolling follow the preference; English and Arabic at desktop, tablet, phone and narrow-phone widths. **Aircraft/photo hero suite:** the flight starts, travels from the correct side, finishes once, and leaves the plane visible, still, inside the hero and clear of everything at every frame, plus the same reduced-motion, no-JavaScript, failing-script and blocked-script cases. Without credentials it runs the suite for the site's current hero and prints `NOT RUN` for the other; with `ADMIN_EMAIL`/`ADMIN_PASSWORD` of an **isolated test site** it switches the hero between the two modes itself and restores it. It needs a browser: with none it says so and exits with code 3, never a pass.
- **Checking it by hand** (if the automated check cannot run): with animations allowed, the **video hero** should start playing by itself within a second or two (on iPhone Safari in Low Power Mode, or with reduced motion or Data Saver, it should instead show the poster and a **Play video** button that works when tapped); the **photo hero** airplane should fly in from the left (English) or right (Arabic) over about 3–4 seconds, land, and stay put. Then, in DevTools, *Rendering → Emulate CSS media feature prefers-reduced-motion → reduce* and reload: no video autoplay and the plane already in place. Finally disable JavaScript and reload: the poster (or the plane) must still be visible.

### Hero video (default hero)

With no CMS hero photograph, the homepage hero plays `public/video/alyas-cloud-flight.mp4` (silent, H.264, 1280×720, fast-start) over `alyas-cloud-flight-poster.webp`. Setting a hero photograph in the CMS replaces the video (see [Hero and photography](#hero-and-photography)).

- **When it plays by itself.** Only when the browser allows muted inline autoplay **and** the visitor has not asked for reduced motion or Data Saver. Otherwise nothing is downloaded (`preload="none"`), the poster stays, and a visible **Play video** button starts it on tap. Browsers such as iPhone Safari in Low Power Mode refuse autoplay by design; that is respected and never worked around.
- **How playback is confirmed.** The Play button and native controls are hidden only once real video frames are advancing, not merely when `play()` resolves. If `play()` is refused, or resolves but no frame arrives within 3.5 seconds, the poster and Play button stay (and the browser's native controls appear). Native controls are also what a visitor gets if JavaScript never runs.
- **Bright video, dark text.** The video is bright, so its header uses `data-tone="hero-bright"`: transparent, with the normal navy text and dark logo (the white header assumes a dark photograph). If you replace the video with a dark one, switch the tone in `src/site/render.tsx` and re-measure contrast.
- **Testing on a phone.** In development the dev server is opened on the phone through the computer's local-network address (for example `http://192.168.0.107:3000`). Next.js blocks development resources for unlisted hostnames, and an un-hydrated page has no working autoplay, so `next.config.ts` lists the private-network ranges in `allowedDevOrigins` (development only; ignored by `next build`/`next start`). Add `?videodebug=1` to the address to show an on-screen log of what the phone's browser reports: reduced-motion and Data Saver state, codec support, every `play()` result (for example `REJECTED NotAllowedError`), media events and whether frames advanced.
- **Making the video silent.** `node scripts/strip-mp4-audio.mjs in.mp4 out.mp4` removes the audio track losslessly (no re-encode) and proves the video samples are byte-identical before writing.
- **Checking it.** `BASE_URL=http://localhost:3000 npm run check:video` (optionally `LAN_URL=…` for the local-network address) inspects the file (fast-start, no audio, codec), byte-range delivery (200/206), and, in a headless Chrome/Edge, that decoded frames advance for autoplay, reduced motion, Data Saver, taps, a failed request, an unsupported codec and JavaScript off, in English and Arabic on phone and desktop. A browser that refuses autoplay is **simulated**, and the check says so; it does **not** replace testing on real iPhones and Androids.
